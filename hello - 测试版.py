...
class Main_window(QDialog):
    ...
    def __init__(self):
        ...
        self.button_choosefile3 = QPushButton("选择筛选文件夹")
        self.cb = QComboBox()
        ...
        self.button_choosefile3.clicked.connect(self.filediropen2_button)
        ...
        layout.addWidget(self.button_choosefile3)
        # 创建一个水平布局
        hbox = QHBoxLayout()
    ...
    def filediropen2_button(self):
        if self.classFileDirPath:
            directory1 = QFileDialog.getExistingDirectory(self, "选择文件夹", self.classFileDirPath)
        elif self.classFileDirPath2:
            directory1 = QFileDialog.getExistingDirectory(self, "选择文件夹", self.classFileDirPath2)
        else:
            directory1 = QFileDialog.getExistingDirectory(self, "选择文件夹", "./")
        if directory1 == "":
            return
        self.classFileDirPath2 = directory1
        print(self.classFileDirPath2)
        self.info2.clear()
        self.info2.setText(directory1)

    def record(self, souce, goal, method):
        workbook = openpyxl.load_workbook("J:\工作簿9.xlsx")
        ...
        # 数据计算
        num_all = 0
        for filename in os.listdir(souce):
            if os.path.splitext(filename)[1] == '.txt':
                num_all = num_all + 1
        num_p = 0
        for filename in os.listdir(goal):
            if os.path.splitext(filename)[1] == '.txt':
                num_p = num_p + 1
        num_t = 0
        for filename in os.listdir(self.classFileDirPath2):
            if os.path.splitext(filename)[1] == '.txt':
                num_t = num_t + 1
        num_tp = 0
        for filename1 in os.listdir(goal):
            for filename2 in os.listdir(self.classFileDirPath2):
                if filename1 == filename2:
                    num_tp = num_tp + 1
        num_fp = num_p - num_tp
        num_tn = num_all - num_t - num_fp
        num_fn = num_all - num_p - num_tn
        sen = num_tp / num_t
        spf = num_tn / (num_tn + num_fp)
        acc = (num_tp + num_tn) / num_all
        ppv = num_tp / num_p
        npv = num_tn / (num_tn + num_fn)
        mcc = (num_tp * num_tn - num_fn * num_fp) / sqrt(
            (num_tp + num_fn) * (num_tp + num_fp) * (num_tn + num_fp) * (num_tn + num_fn))
        print(num_all, num_p, num_t)
        # 写入数据
        hh = [souce, method, num_all, num_t, num_all / num_t, num_p, num_tp, num_fp, num_tn, num_fn, sen, spf, acc, ppv,
              npv, mcc]
        worksheet.append(hh)
        workbook.save(filename="J:\工作簿9.xlsx")
        # 删除数据
        for filename in os.listdir(goal):
            os.remove(goal + "\\" + filename)
        os.removedirs(goal)
    ...
    def mystart(self):
        ...
        if self.judgeMethod == 4:
            ...
            # self.judgeMode = 1
            # for i in range(10):
            #      self.classFileDirPath = 'J:\毕业设计\\20190402\分解数据' + str(i)
            #      self.classFileDirPath2 = 'J:\毕业设计\\20190402\分解数据' + str(i) + '\筛选数据' + str(i)
            #      self.test_click()
        ...
        # 判断文件夹内容
            ...
            self.text_judge2.setText(
                "文件夹共有" + str(all_read_file_num) + "个文件，其中" + str(right_read_file_num) + "个是目标文件," + str(
                    wrong_file_num) + "个非法文件")
            self.judgeMode = 1
            self.record(self.classFileDirPath, self.classFileDirPath + '\目标数据_svm', "SVM")
        ...
        # 判断文件夹内容
            ...
            self.text_judge2.setText(
                "文件夹共有" + str(all_read_file_num) + "个文件，其中" + str(right_read_file_num) + "个是目标文件," + str(
                    wrong_file_num) + "个非法文件")
            self.judgeMode = 1
            self.record(self.classFileDirPath, self.classFileDirPath + '\目标数据_决策树', "决策树")
        curr_time5 = datetime.datetime.now()
        print(curr_time5)
        self.setMode_3(0)
    ...
    def method3_click(self):
        ...
        if self.judgeMode == 0:
            ...
            filename = self.classFilePath
            retract = self.read_txt(filename)
            judge_restlt = self.forest_method(retract, filename, '.\目标数据_随机森林')
        ...
        # 判断文件夹内容
            ...
            self.text_judge2.setText(
                "文件夹共有" + str(all_read_file_num) + "个文件，其中" + str(right_read_file_num) + "个是目标文件," + str(
                    wrong_file_num) + "个非法文件")
            self.judgeMode = 1
            self.record(self.classFileDirPath, self.classFileDirPath + '\目标数据_随机森林', "随机森林")
        curr_time5 = datetime.datetime.now()
        print(curr_time5)
        self.setMode_3(0)
    ...
    def test_click(self):
        ...
        if self.judgeMode == 0:
            ...
            filename = self.classFilePath
            retract = self.read_txt(filename)
            judge_restlt = self.test_svm_method(retract, filename, '.\目标数据_test')
        ...
        # 判断文件夹内容
            ...
            for filename in os.listdir(self.classFileDirPath):
                ...
                if os.path.splitext(filename)[1] == '.txt':
                    ...
                    if retract == 1:
                        ...
                        judge_result = self.test_all2_method(retract, self.classFileDirPath + '\\' + filename,
            ...
            self.text_judge2.setText(
                "文件夹共有" + str(all_read_file_num) + "个文件，其中" + str(right_read_file_num) + "个是目标文件," + str(
                    wrong_file_num) + "个非法文件")
            self.judgeMode = 1
            self.record(self.classFileDirPath, self.classFileDirPath + '\目标数据_test', "test2 pca15")
        curr_time5 = datetime.datetime.now()
        print(curr_time5)
        self.setMode_3(0)
    ...
    # 方法4：
    def test_forest_method(self, retract, sourcefile, target):
        vecdata = self.deal(retract)
        # load PCA模型，降维
        if self.shapeMode == 0:
            pca = joblib.load('pca_4.pkl')
            xdata = pca.transform(vecdata)
            # load 决策树模型
            model1 = joblib.load('model_forest_40.pkl')
            ypred1 = model1.predict(xdata)
            model2 = joblib.load('model_forest_41.pkl')
            ypred2 = model2.predict(xdata)
            model3 = joblib.load('model_forest_42.pkl')
            ypred3 = model3.predict(xdata)
            model4 = joblib.load('model_forest_43.pkl')
            ypred4 = model4.predict(xdata)
        else:
            pca = joblib.load('pca_8.pkl')
            xdata = pca.transform(vecdata)
            # load 决策树模型
            model1 = joblib.load('model_forest_4.pkl')
            ypred1 = model1.predict(xdata)
            model2 = joblib.load('model_dtree_4.pkl')
            ypred2 = model2.predict(xdata)
            model3 = joblib.load('model_svm_4.pkl')
            ypred3 = model3.predict(xdata)
            model4 = joblib.load('adaboost_4.pkl')
            ypred4 = model4.predict(xdata)
        # 复制目标数据txt文件
        if ypred1[0] + ypred2[0] + ypred3[0] + ypred4[0] > 2:
            # if self.shapeMode == 0:
            #     pca = joblib.load('pca_4.pkl')
            #     xdata = pca.transform(vecdata)
            #     # load 决策树模型
            #     model = joblib.load('adaboost_4.pkl')
            #     ypred = model.predict(xdata)
            # else:
            #     pca = joblib.load('pca_8.pkl')
            #     xdata = pca.transform(vecdata)
            #     # load 决策树模型
            #     model = joblib.load('adaboost_8.pkl')
            #     ypred = model.predict(xdata)
            # if ypred[0] == turelabel:
            shutil.copy(sourcefile, target)  # 复制txt文件
            print(sourcefile)
    ...
    def test_dtree_method(self, retract, sourcefile, target):
        vecdata = self.deal(retract)
        # load PCA模型，降维
        if self.shapeMode == 0:
            pca = joblib.load('pca_4.pkl')
            xdata = pca.transform(vecdata)
            # load 决策树模型
            model1 = joblib.load('model_dtree_40.pkl')
            ypred1 = model1.predict(xdata)
            model2 = joblib.load('model_dtree_41.pkl')
            ypred2 = model2.predict(xdata)
            model3 = joblib.load('model_dtree_42.pkl')
            ypred3 = model3.predict(xdata)
            model4 = joblib.load('model_dtree_43.pkl')
            ypred4 = model4.predict(xdata)
        else:
            pca = joblib.load('pca_8.pkl')
            xdata = pca.transform(vecdata)
    ...
    def test_svm_method(self, retract, sourcefile, target):
        vecdata = self.deal(retract)
        # load PCA模型，降维
        if self.shapeMode == 0:
            pca = joblib.load('pca_4.pkl')
            xdata = pca.transform(vecdata)
            # load 决策树模型
            model1 = joblib.load('model_svm_40.pkl')
            ypred1 = model1.predict(xdata)
            model2 = joblib.load('model_svm_41.pkl')
            ypred2 = model2.predict(xdata)
            model3 = joblib.load('model_svm_42.pkl')
            ypred3 = model3.predict(xdata)
            model4 = joblib.load('model_svm_43.pkl')
            ypred4 = model4.predict(xdata)
        else:
            pca = joblib.load('pca_8.pkl')
            xdata = pca.transform(vecdata)
    ...
    def test_all_method(self, retract, sourcefile, target):
        rightnum = 2
        vecdata = self.deal(retract)
        pca = joblib.load('pca_4.pkl')
        xdata = pca.transform(vecdata)
        ...
        model4 = joblib.load('model_svm_43.pkl')
        ypred4 = model4.predict(xdata)
        if ypred1[0] + ypred2[0] + ypred3[0] + ypred4[0] > rightnum:
            model1 = joblib.load('model_dtree_40.pkl')
            ypred1 = model1.predict(xdata)
            model2 = joblib.load('model_dtree_41.pkl')
            ypred2 = model2.predict(xdata)
    ...
    def test_all2_method(self, retract, sourcefile, target):
        vecdata = self.deal(retract)
        if self.shapeMode == 0:
            num_of_peak = str(4)
        else:
    ...
    def test1_method(self, retract, sourcefile, target):
        vecdata = self.deal(retract)
        pca = joblib.load('pca_4.pkl')
        xdata = pca.transform(vecdata)
