...
def read_txt(filename):  # 读txt文件，传回拉伸段y数据，排好序的
    ...
    # min_x = np.min(retract_x)
    # retract_x = [num - min_x for num in retract_x]
    ...
    #     orderxy = random.sample(retract, min_length)
    #     orderxy.sort(key=lambda x: x[0], reverse=True)
    #     ordery = [x[1] for x in orderxy]
    # else:
    #     vecdata = [x[1] for x in retract]  # 排好序后的y值
    #     for i in range(min_length):
    #         ordery.append(vecdata[int(i * data_length / min_length)])
    # y_min=np.min(ordery)
    # y_max=np.max(ordery)
...
def svm_train(train_data, train_label, test_data, test_label):
    ...
    #joblib.dump(clf, 'model_svm_4.pkl')  # 保存模型
...
def decision_tree(train_data, train_label, test_data, test_label):
    ...
    #joblib.dump(clf, 'model_dtree_4.pkl')  # 保存模型
    test_scores = clf.score(test_data, test_label)  # 测试集得分
    print("得分为", test_scores)
    start = time.time()
    y_pred = clf.predict(test_data)  # 输出测试集信息，准确率、召回率等
...
def Random_forest(train_data, train_label, test_data, test_label):
    ...
    print('随机森林训练', end - start)
    #joblib.dump(clf, 'model_forest_4.pkl')  # 保存模型
    test_scores = clf.score(test_data, test_label)  # 测试集得分
    print("得分为", test_scores)
    start = time.time()
    y_pred = clf.predict(test_data)  # 输出测试集信息，准确率、召回率等
    ...
    return clf
# adaboost训练
def adaboost(train_data, train_label, test_data, test_label):
    clf=AdaBoostClassifier(n_estimators=100, learning_rate=0.1)
    start = time.time()
    clf.fit(train_data, train_label)  # 训练
    end = time.time()
    print('adaboost训练', end - start)
    #joblib.dump(clf, 'adaboost_4.pkl')  # 保存模型
    test_scores = clf.score(test_data, test_label)  # 测试集得分
    print("得分为", test_scores)
    start = time.time()
    y_pred = clf.predict(test_data)  # 输出测试集信息，准确率、召回率等
    ...
    return clf
# 训练获得模型
targetfiledir = 'train_4'  # 目标数据文件夹
nottargetfiledir = 'train_4.2原始'  # 非目标数据文件夹
x_ture, y_ture = read_train(targetfiledir, 1)  # 读入目标数据和标签
x_flase, y_flase = read_train(nottargetfiledir, 0)  # 读入非目标数据和标签
x_train=[]
y_label=[]
for i in range (num_cls):
    label=y_ture[0:26]+y_flase[0:399]
    y_label.append(label)
    train=x_ture[27*i:27*i+26]+x_flase[400*i:400*i+399]
    x_train.append(train)
del x_ture,y_ture,x_flase,y_flase
...
test_label=[]
for i in range (num_cls):
    x,y=SMOTE().fit_sample(x_train[i], y_label[i])
    X_resample.append(x)
    y_resample.append(y)
    train_d, test_d, train_l, test_l = sklearn.model_selection.train_test_split(X_resample[i],y_resample[i],train_size=0.7, test_size=0.3)
    train_data.append(train_d)
    test_data.append(test_d)
    train_label.append(train_l)
    test_label.append(test_l)
    print(Counter(y_resample[i]))
pca = PCA(n_components=components)  # 建立pca模型
  # 训练并降维
# ptest_data = pca.transform(test_data)
# joblib.dump(pca, 'pca_4.pkl')  # 保存模型
clfs=[]
for i in range(num_cls):
    ptrain_data = pca.fit_transform(train_data[i])
    ptest_data = pca.transform(test_data[i])
    clfs.append(svm_train(ptrain_data, train_label[i], ptest_data, test_label[i]))
eclf = EnsembleVoteClassifier(clfs, voting='soft')
joblib.dump(eclf, 'model_svm_4.pkl')
for i in range(num_cls):
    ptrain_data = pca.fit_transform(train_data[i])
    ptest_data = pca.transform(test_data[i])
    clfs[i]=decision_tree(ptrain_data, train_label[i], ptest_data, test_label[i])
eclf = EnsembleVoteClassifier(clfs, voting='soft')
joblib.dump(eclf, 'model_dtree_4.pkl')
for i in range(num_cls):
    ptrain_data = pca.fit_transform(train_data[i])
    ptest_data = pca.transform(test_data[i])
    clfs[i]=Random_forest(ptrain_data, train_label[i], ptest_data, test_label[i])
eclf = EnsembleVoteClassifier(clfs, voting='soft')
joblib.dump(eclf, 'model_forest_4.pkl')
for i in range(num_cls):
    ptrain_data = pca.fit_transform(train_data[i])
    ptest_data = pca.transform(test_data[i])
    clfs[i]=Random_forest(ptrain_data, train_label[i], ptest_data, test_label[i])
eclf = EnsembleVoteClassifier(clfs, voting='soft')
joblib.dump(eclf, 'model_adaboost_4.pkl')
# train_data1, test_data1, train_label1, test_label1 = sklearn.model_selection.train_test_split(X_resample1, y_resample1,
#                                            train_size=0.7, test_size=0.3)
# train_data2, test_data2, train_label2, test_label2 = sklearn.model_selection.train_test_split(X_resample2, y_resample2,
#                                            train_size=0.7, test_size=0.3)
# train_data3, test_data3, train_label3, test_label3 = sklearn.model_selection.train_test_split(X_resample3, y_resample3,
#                                            train_size=0.7, test_size=0.3)
# train_data4, test_data4, train_label4, test_label4 = sklearn.model_selection.train_test_split(X_resample4, y_resample4,
#                                            train_size=0.7, test_size=0.3)
...
# print(4)
# Random_forest(ptrain_data, train_label, ptest_data, test_label)
# print(5)
# adaboost(ptrain_data, train_label, ptest_data, test_label)
# 决策树模型导出文件，使用graphviz工具可以可视化
dtree = joblib.load('model_dtree_4.pkl')
with open("tree_4.dot", 'w') as f:
