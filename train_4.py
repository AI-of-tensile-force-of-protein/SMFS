...
def read_txt(filename):  # 读txt文件，传回拉伸段y数据，排好序的
    ...
    # 当前的取点为如果点数在4000以下，则按比例调整至4000，4000以上则随机取4000点
    # if min_length<=data_length:
    #     orderxy=random.sample(retract,min_length)
    #     orderxy.sort(key=lambda x: x[0], reverse=True) # 二维点按x降序排序
...
nottargetfiledir = 'train_4原始'  # 非目标数据文件夹
...
# 随机划分训练集与测试集
#x,y=ADASYN().fit_sample(x_train, y_label)
train_data, test_data, train_label, test_label = sklearn.model_selection.train_test_split(x_train, y_label,
