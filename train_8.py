...
def svm_train(train_data, train_label, test_data, test_label):
    ...
    joblib.dump(clf, 'model_svm_8.pkl')  # 保存模型
...
def decision_tree(train_data, train_label, test_data, test_label):
    ...
    joblib.dump(clf, 'model_dtree_8.pkl')  # 保存模型
    test_scores = clf.score(test_data, test_label)  # 测试集得分
    print("得分为", test_scores)
    start = time.time()
    y_pred = clf.predict(test_data)  # 输出测试集信息，准确率、召回率等
...
def Random_forest(train_data, train_label, test_data, test_label):
    ...
    joblib.dump(clf, 'model_forest_8.pkl')  # 保存模型
    test_scores = clf.score(test_data, test_label)  # 测试集得分
    print("得分为", test_scores)
    start = time.time()
    y_pred = clf.predict(test_data)  # 输出测试集信息，准确率、召回率等
    ...
    print(classification_report(test_label, y_pred))
# 训练获得模型
targetfiledir = 'train_8'  # 目标数据文件夹
nottargetfiledir = 'train_8原始'  # 非目标数据文件夹
...
# 随机划分训练集与测试集
x,y=SMOTE().fit_sample(x_train, y_label)
...
joblib.dump(pca, 'pca_8.pkl')  # 保存模型
...
Random_forest(ptrain_data, train_label, ptest_data, test_label)  # 训练随机森林模型
# 决策树模型导出文件，使用graphviz工具可以可视化
dtree = joblib.load('model_dtree_8.pkl')
with open("tree_8.dot", 'w') as f:
